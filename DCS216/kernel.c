...
uint8_t count_executables() {
  ...
  uint8_t err = load_sector(0x0900, 0x0000, 0x00, 10, 1);
  if (err) {
  ...
  for (uint8_t i = 0; i < 0x0200; i++) {
    ...
    int8_t value;
    do {
      value = raw_far_pointer_read(0x0900, i);
...
void run(uint8_t id) {
  int8_t disc = syscall_get_default_drive();
  uint8_t err = load_sector(0x0900, 0x0100, 0x00, id * 4 + 13, 4);
  if (err) {
    print_str(error_occurred);
    print_u8_hex(err);
    print_str("\n\r");
